...
def get_num_agents(num_tasks):
    with open('./data/task_'+str(num_tasks)+'.json', 'r') as f:
        data = json.load(f)
    return len(data)


# 更新id_list 中对应id在 round时隙的位置信息
def get_final_np(current_time_str, max_id, id_list,num_tasks):
    result_array = []
    result = []
...
def add_task(task_id, cur_time_str, length, max_id,num_tasks):
    with open('./data/task_'+str(num_tasks)+'.json', 'r') as file:
