...
class CustomEnv(gym.Env):
    def __init__(self, num_agents, num_satellites,num_tasks=150,algo=''):
    ...
    def reset(self):
        ...
        # 初始调度150个任务
        self.task_id = self.num_agents
        self.task_idx_np = np.arange(1, self.num_agents+1)
        self.already_finished = 0
        self.final_already_finished = 0
        self.json_read = get_final_np("2024-07-19T00:00:00Z", self.num_satellites, self.task_idx_np,self.num_tasks)
        self.state = np.hstack((self.actions, self.json_read))

        self.reward_file = open('output/rewards_'+self.algo+'_'+str(self.num_tasks)+'.txt', 'a', buffering=1)
        self.connection_file = open('output/connections_'+self.algo+'_'+str(self.num_tasks)+'.txt', 'a', buffering=1)
    ...
    def step(self, actions, **kwargs):
        ...
        self.json_read = get_final_np(current_time_str, self.num_satellites, self.task_idx_np,self.num_tasks)
        ...
        for idx, item in enumerate(self.done):
            if item == 1.0:
                tmp = add_task(self.task_id, current_time_str, 1, self.num_satellites,self.num_tasks)
                if len(tmp) == 1:  # 能够增加新task
                    ...
                    self.task_id = get_num_agents(self.num_tasks)
                    self.agent_use_np[idx] = 1  # 该agent已经闲置
                    self.final_already_finished = self.already_finished + np.sum(self.agent_use_np)

        self.last_actions = self.actions
        # 读取下一个时隙的拓扑信息
        self.json_read = get_final_np(next_time_str, self.num_satellites, self.task_idx_np,self.num_tasks)
        # 更新k
        new_connections = np.zeros(self.num_satellites, dtype=float)  # new_connections用于存储每个卫星的k
        for agent_id in range(self.num_agents):
            # 对于本次step不执行动作的agent，特殊处理
